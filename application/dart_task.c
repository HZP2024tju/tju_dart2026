...
float dart_2006_angle_set = -44.2;//-200                    //       46.8469658        3  -50.5     5    -52            1  61.3     3 - 59.5        5 - 61     6  -61             3 -57.8  5  -59.4      6   -57.5    2 -56.8;                       //     5  73   3   71.4;3和5号可以认为一致
																			
//-4 约为35cm  bias
fp32 Yaw_Angle = -87.38;//-70;//-85;               //修改Yaw轴方向,越负越往右
...
uint8_t dart_target = 1;   //前哨为0 基地为1
         ...
         if(shoot_flag_renew->Shoot_Mode == SHOOT_RELOADING)
				...
				if(fabs(shoot_flag_renew->Dart_2006_Bullet_Motor.speed)<0.3f && shoot_flag_renew->Dart_Flag.Reset_Bullet_2006.flag == 0)  //用堵转判定复位 或许有一天会改为电流判定,让大一的写
				{
						...
						//判断偏转2006是否复位
							...
							if(fabs(shoot_flag_renew->Dart_Yaw_Motor.speed)<0.3f && shoot_flag_renew->Dart_Flag.Reset_Yaw_2006.flag == 0) 
							{
      ...
      {	
				...
				case SHOOT_RELOADING:
				{
					...
					else if(auto_start == 1)
					{
						...
						{
								set_control->Dart_3508_Angle_Pid.MaxOut = BULLET_3508_ANGLE_PID_MAX_OUT/2.0;       //不能上升太快,不然在力度大的情况下同步带脱齿,3508定位不准
					...
					if(set_control->dart_reload_step == SERVO_PULL)  //改为速度pid
					{
		...
		{		
			 control_loop->Dart_6020_Motor.speed_set = PID_Calculate(&control_loop->Dart_6020_Angle_Pid,
				...
				if( control_loop->Shoot_Mode == SHOOT_UPING)//3508复位
				{
						...
						control_loop->Dart_3508_Motor.speed,control_loop->Dart_3508_Motor.speed_set),-3000,3000);						                      //输出限幅 以此改变上弹的力 16384
				}
...
//control_loop->Dart_Yaw_Motor.give_current = 0;							
						...
						control_loop->Dart_Yaw_Motor.give_current = int16_constrain((int16_t)PID_Calculate(&control_loop->Yaw_Motor_Gyro_Pid,
																		control_loop->Dart_Yaw_Motor.speed,control_loop->Dart_Yaw_Motor.speed_set),-4000,4000);					        //速度PID    首先回正
						}
